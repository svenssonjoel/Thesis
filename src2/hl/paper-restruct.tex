...
% --------------------------------------------------------------------------- 
\section{A taste of Obsidian programming}
...
while a push array captures scattering operations. Reference \cite{Obsidian} 
...
embedded language Pan ~\cite{ELLIJFP} used a similar representation for 
...
% --------------------------------------------------------------------------- 
\subsection{Thread, Block and Grid Computations} 
\label{sec:Program}
...
exposed by Obsidian. In reference ~\cite{CSORT} we make use of these low-level 
...
% Interplay 
% --------------------------------------------------------------------------- 
\subsection{Push and pull array interplay} 
\label{sec:interplay}
...
tutorial presentation from NVIDIA~\cite{RedOpt}.
...
\subsubsection{Reductions 6 and 7}
...
\subsubsection{Larger local reductions} 
...
\subsubsection{two elements per thread}
...
Similar patterns were used in our implementations of sorting networks~\cite{Obsidian}, for similar reasons.
...
\section{Combining kernels to solve large problems} 
...
~\cite{accelerate}. The abstraction level is comparable to that of Thrust. In 
...
continuation monad by Persson et al~\cite{Generic}. Recent work by Sculthorpe et al. in reference ~\cite{NEIL} is also related. 
...
\subsection{Increase low level control} 
...
\subsection{To automate or not to automate} 
...
\subsection{To optimise or not to optimise} 
...
\subsection{Kernel Coordination} 
...
used in the NVIDIA tutorial ~\cite{RedOpt}. The case study also show hows we 
...
%in more detail in reference ~\cite{BB}. 
