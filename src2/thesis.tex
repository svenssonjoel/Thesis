...
In 2006 NVIDIA released their first Compute Unified Device Architechture (CUDA) 
GPU. With CUDA, NVIDIA also made it a lot easier for programmers to use GPUs 
for general purpose workloads. A dialect of C, called CUDA C, was released specifically 
to simplify implementation of general purpose algorithms on GPUs. 

A CUDA GPU is built around a single kind of processor (opposed to the different 
...
by all of the SPs in that MP. The number of MPs vary over the available GPUs; cheaper 
GPUs have as few as one MP, and as you go up in price the number of MPs increase.
...
a {\em Warp}, that are executed in lock-step (SIMD style execution). Threads are 
also divided into {\em Blocks}; the threads within a block can communicate using the 
local shared memory. The maximum block size is currently 1024 threads. Since 
...
about them ,in relation to Obsdian. 

\subsection{CUDA programming} 

The CUDA programming model reflects the hierarchical architecture of GPUs. The programmer 
writes what is called {\em kernels}, in CUDA jargon a kernel refers to a sequential C program 
that is parameterised on a threads identity. The kernel program is executed in parallel across 
all the threads of a block and many such blocks of threads can execute in parallel on the 
available MPs. The programmer needs to decompose the algorithm and data in such a way what 
...
__global__ void kernel(int32_t *i1, int32_t *i2, int32_t *r) {
  ...
  r[threadIdx.x] = i1[threadIdx.x] + i2[threadIdx.x]; 
...
elementwise sums into the result array, {\tt r}. {\tt threadIdx} identifies 
a thread within a block. There is also a {\tt blockIdx} variable that identifies 
the block a thread belongs to but that is not used in this case. 
...
the host systems memory into device memory. 
...
In this case one block of 32 threads using no shared memory, is launched. 
...
wider context surrounding Obsidian is described in section~\ref{sec:relatedwork}. 
...
be able to use local shared memory in computations and has the ability to influence 
...
within a warp and is then free to remove synchronisations. If warps where to be put in the 
program level hierarchy of Obsidian, its place would be between the thread and block level. 
This forces the programmer to go via warps when putting together a block computation even 
...
The approach taken to embedd the scalar language is very similar to what is described 
in~\citet{COMPILEEDSL}; but we use a Generalised Algebraic Datatype (GADT) to obtained 
typed expressions rather than phantom types. 
...
Push arrays where added to Obsidian as a complement to pull arrays. Pull arrays are 
