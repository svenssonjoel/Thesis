...
Obsidian and I modified the code generator to accomodate them. Sections 
...
%Intel has unfortunately retired the ArBB project. This leaves EmbArBB without a fuctioning 
...
This also means that it is becoming important to think in parallel from the beginning. 
...
system approach this problem by providing a high level programming interface~\citet{ARBB2011}.
...
naive reduction kernel is implement and it is refined over many steps, taking many 
characteristics of the GPU into mind. The end result is a reduction kernel that performs 
very well on the chosen GPU. But as GPUs evolve the design space may need to be explored again. 
%What is 
...
ability to generate code for a variety of different hardware confgurations. It is implemented 
as an embedded language in C++. One motivation for ArBB that is mentioned in~\citet{ARBB2011} is 
that while high-performance computing specialist are highly competent at implementing 
...
ArBB is based on a set of parallel primitives (or structures) on dense 1,2 and 3 dimensional 
...
\item {\bf Sequential controlflow} {\tt \_for}, {\tt \_while} 
\item {\bf Map, zipWith, stencil}  {\tt map} 
\end{itemize}

These operations (and many more, there are about 120 operations in total) can be used 
by the programmer when writing programs using ArBB. Note that there are sequential control 
operations with special names, {\tt \_for} and {\tt \_while}. ArBB is a deeply embedded 
...
This is exactly the way these things work in Haskell but it may be even more surprising to a
C++ programmer. Before an ArBB function can be computed it needs to be captured. For this 
ArBB has a {\tt call} function that is used to run functions using ArBB functionality. The 
first time {\tt call} is used on a function pointer that function is executed and the ArBB 
functions used in it builds an AST (we say the function has been captured). The AST is then 
compiled and specialised for the hardware available. Any subsequent calls of a function using 
ArBB functionality does not lead to a recompilation, the first call caches this code. 
...
more general language and there was an API missmatch between them. These problems 
...
\emph{Sharing detection} 

\emph{let ArBB do the optimisations} 

\emph{having deep embedding means we could compile to something else} 
...
leaves EmbArBB without a functioning code generating backend. 
...
This thesis contributes to the follwoing areas: 

\begin{itemize} 
\item Embedded language design and implementation. 
\item General Purpose programming using a GPU (GPGPU).
\item High level data-parallel programming across devices.
\end{itemize} 

\subsubsection{Embedded language design and implementation} 


\subsubsection{General Purpose programming using a GPU} 


\subsubsection{High level data-parallel programming across devices}
