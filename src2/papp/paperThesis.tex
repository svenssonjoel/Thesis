...
passed to the {\tt nvcc} compiler~\citepapp{CUDAGuide2.0}. Currently, we 
...
%compiler \citepapp{CUDAGuide2.0}. 
...
of threads \citepapp{Brief8800}. The Obsidian project is about exploring ways 
...
In~\citepapp{JSMSKC_IFL08} we describe a version of Obsidian that uses a monadic 
interface. In the current version of Obsidian the monad has been replaced 
by another data structure, closely related to Arrows~\citepapp{Arrows}, representing GPU programs. Although the details are 
...
see~\citepapp{JSTECH} and for more general information on embedded language 
implementation see~\citepapp{COMPILEEDSL}.
...
\citepapp{CUDAGuide2.0}. CUDA offers the GPGPU programmer a C %,wwwcuda,Brief8800
...
%the two diverging paths will in fact be executed sequentially \citepapp{BestPrac}. 
...
verification language \citepapp{lavaICFP}. The justification to use language 
...
pictures, see for example \citepapp{GEMS3}. 
...
%specific array sizes \citepapp{GEMS3}. In a later stage, when the kernels are 
...
The type constructor {\tt :->} is a variant of an arrow, which in turn is a generalisation of a monad~\citepapp{JSTECH}.
%
\begin{figure}
\begin{center}
...
Sorting is a popular function to place on the GPU, see~\citepapp{SINTORNSORT,SORT3}.
...
kernel, known as {\tt sklansky} after J. Sklansky \citepapp{Sklansky}.
...
For a more thorough explanation of {\tt syncHow} see ~\citepapp{JSTECH}.
For an explanation of the parallel-prefix operation see~\citepapp{BlellochTR90}. 
...
implementation is shown in~\citepapp{ScanCUDA}. The {\tt NVIDIA SDK} 
...
\citepapp{CUDAGuide2.0}. CUDA supplies a slightly extended version of C in which 
...
GPGPU programmers, for example Brook\citepapp{Brook} and 
RapidMind\citepapp{RapidMind}. Brook, CUDA and RapidMind are major improvements from 
...
programming language in Python\citepapp{PyGPU}. PyGPU makes use of Python's 
introspective abilities to generate efficient code. 

Like Obsidian, Accelerate is embedded in Haskell \citepapp{GPUGEN}. Accelerate 
...
Vertigo is another GPU programming language embedded in Haskell\citepapp{VERTIGO}.
...
Of particular interest to us is Microsoft's Accelerator project~\citepapp{ACCELERATOR}. Interestingly, Accelerator includes the same restriction to ``hardware-like'' algorithms as Obsidian does.
...
%on parallel arrays\citepapp{DPH}. Data-Parallel Haskell implements the nested 
...
%\begin{itemize}
%\item Chapel \citepapp{Chapel}
%\item DPH \citepapp{DPH}
%\item Fortress \citepapp{fortress}
%\item GPUGen \citepapp{GPUGEN}
%\item Lava \citepapp{lavaTutorial}
%\item RapidMind \citepapp{RapidMind}
%\item Sequoia \citepapp{SEQUOIA}
%\item X10 \citepapp{X10}
%\end{itemize}
...
\bibliographystylepapp{alpha}
\bibliographypapp{thesis}
